...
{
  gtk_widget_set_sensitive (app->file_chooser, !running);
...
on_bus_message (GstBus * bus, GstMessage * message, gpointer user_data)
{
  ...
  switch (GST_MESSAGE_TYPE (message)) {
    ...
    case GST_MESSAGE_EOS:
      app->bus_watch_id = 0;
      stop_pipeline (app,
          "File completed. The UDP receiver remains active until it is stopped.");
      return G_SOURCE_REMOVE;
...
start_pipeline (DemoApp * app)
{
  ...
  if (app->pipeline == NULL || source == NULL || demux == NULL ||
      app->sink == NULL) {
    gtk_label_set_text (GTK_LABEL (app->status_label),
        app->sink == NULL ?
  ...
  gst_bin_add_many (GST_BIN (app->pipeline), source, demux, app->sink, NULL);
...
build_ui (DemoApp * app)
{
  ...
  description = gtk_label_new (
      "Streams the H.264 or H.265 video track from an MP4 file as RTP. "
  ...
  gtk_grid_attach (GTK_GRID (settings), app->file_chooser, 1, 0, 3, 1);

  label = gtk_label_new ("Destination");
  gtk_widget_set_halign (label, GTK_ALIGN_END);
  gtk_grid_attach (GTK_GRID (settings), label, 0, 1, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (settings), app->host_entry, 1, 1, 1, 1);

  label = gtk_label_new ("Port");
  gtk_widget_set_halign (label, GTK_ALIGN_END);
  gtk_grid_attach (GTK_GRID (settings), label, 2, 1, 1, 1);
  port_adjustment = gtk_adjustment_new (5004, 1, 65535, 1, 100, 0);
  app->port_spin = gtk_spin_button_new (port_adjustment, 1, 0);
  gtk_grid_attach (GTK_GRID (settings), app->port_spin, 3, 1, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (settings), app->mode_combo, 1, 2, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (settings), app->batch_spin, 3, 2, 1, 1);
  ...
  gtk_grid_attach (GTK_GRID (settings), app->fallback_check, 1, 3, 3, 1);
